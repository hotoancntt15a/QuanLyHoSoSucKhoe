...
namespace QuanLyHoSoSucKhoe.Areas.danhmuc.Controllers
{
    ...
    {
        private string tableName = "dmquocgia";
        // GET: danhmuc/dantoc
        [Properties(Name = "Quốc gia", Note = "fa fa-list")]
        ...
        public ActionResult timkiem()
        {
            ...
            if (string.IsNullOrEmpty(tmp) == false) { where.Add(tmp.buildLikeSql("quocgia")); }
            tmp = where.Count > 0 ? " where " + string.Join(" and ", where) : "";
            var tsql = $"select * from {tableName} " + tmp + " order by quocgia";
            using (var db = local.getDataObject())
            {
        ...
        public ActionResult capnhat()
        {
            ...
            if (Request["mode"] != null)
            {
                ...
                try
                {
                    ...
                    data["quocgia"] = Request.getValue("quocgia").Trim();
                    data["ghichu"] = Request.getValue("ghichu").Trim();
                    if (string.IsNullOrEmpty(data["id"])) { throw new Exception("Tham số không đúng: mã"); }
                    if (string.IsNullOrEmpty(data["quocgia"])) { throw new Exception("Tham số không đúng: tên quốc gia"); }
